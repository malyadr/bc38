...
export const getAllJobsBySkillsAndLocation = createAsyncThunk(
    'jobs/fetchAlljobsBySkillsAndLocation',
    async ({ skills, location }: SearchProps) => {
        try {
            const { data } = await axios.get(`${BASE_URL}`)
            let response = data
...
export const getAllSavedJobs = createAsyncThunk(
    ...
    async () => {
        try {
            const { data } = await axios.get(`${BASE_URL}`)
            return data.filter((job: JOBCARDPROPS) => true)
        } catch (err) {
            console.log(err)
        }
    }
...
export const getAllFilteredJobs = createAsyncThunk(
